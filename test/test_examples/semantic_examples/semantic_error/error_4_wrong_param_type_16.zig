// Semantic error: Wrong parameter type of function "ifj.i2f()" on line 14.2
...
{ // 3
  ...
  if(x<10) { // 5
    ...
    ifj.write("\n"); // 12
    const res = g(y); // 13
    const floating : f64 = 3.14;
    var convert = ifj.i2f(floating); // 14.2
    convert = convert + 10.857; // 14.3
    return res; // 14.4
  } // 15
} // 16
